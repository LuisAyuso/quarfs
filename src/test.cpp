...
/* index
 * line  265: translation buffer setup
 * line  336: draw call
 */
...
int main() {
   ...
   // start GL context and O/S window using the GLFW helper library
    ...
    auto window = glfwCreateWindow (width, height, "hello", NULL, NULL);
    if (!window) {
...
//    glfwSetMouseButtonCallback (window, mouseCallback);
    ...
    // note: one buffer for vertex and color!!!!!

    // vertex! stride 6 floats offset 0
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6*sizeof(GLfloat), (char*)0 + 0*sizeof(GLfloat));

    // colors same buffer, stride 6 floats offset 3 (second half)
    ...
    // fill with data (note: element array !! )
